...
class PhotoAlbumViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, MKMapViewDelegate, NSFetchedResultsControllerDelegate {
    ...
    func getPhotos () {
        FlickrClientUser.getPhotosFromLocation(latitude: pinLocations?.latitude ?? 0.0, longitude: pinLocations?.longitude ?? 0.0, page: page, completion: { (response, error) in
            if let response = response {
                ...
                for photo in downloadedImageUrls {
                    ...
                    //server: String, id: String, secret: String
                    newPhoto.coreURL = FlickrClientUser.getPhotoURL(server: photo.server, id: photo.id, secret: photo.secret)
                    try? self.dataController.viewContext.save()
                }
