...
class FlickrClientUser {
    ...
    enum Endpoints {
        ...
        var stringValue: String {
            ...
            case .getPhotoURL(let server, let id, let secret):
                return Endpoints.flickrPhotos + "\(server)/\(id)_\(secret).jpg"
            }
        }
    ...
    class func getPhotosFromLocation (latitude: Double, longitude: Double, page: Int, completion: @escaping (FlickrResponses?, Error?)-> Void) { taskForGETRequest(url: Endpoints.getPhotos(latitude,longitude, page).url, responseType: FlickrResponses.self) { response, error in
